...
### 1. `env` (Environnement)
* **Gestionnaire central** : Il suit l'état des populations (prédateurs, proies, herbe) et les conditions climatiques.
...
* **Autonomie** : Chaque individu possède ses propres attributs comme l'énergie et un état (actif ou passif).
...
* **Mort** : Un processus se termine et notifie l'environnement si l'énergie de l'individu tombe à zéro.
...
* **Contrôle** : Permet d'initialiser les populations au démarrage et d'envoyer des signaux pour modifier l'environnement (sécheresse).
...
| **Signals (SIGUSR1)** | Déclenchement instantané d'une sécheresse envoyé du processus `display` vers `env`. |
